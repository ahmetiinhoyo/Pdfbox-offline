// Russian translations
export const ru = {
    tagline: "Ваш файл не покидает устройство. Без рекламы и слежки.",
    navMerge: "Объединить",
    navSplit: "Разделить",
    navImages: "Изображения в PDF",
    badgeOffline: "100% офлайн",
    badgeNoUpload: "Без загрузки",
    badgeNoTrack: "Без слежки",
    navPages: "Редактор страниц",
    pagesMoveLeft: "Влево",
    pagesMoveRight: "Вправо",
    pagesTitle: "Редактор страниц",
    pagesDesc: "Удаляйте и переставляйте страницы PDF и скачивайте результат одним файлом.",
    pagesDropText: "Нажмите или перетащите PDF",
    pagesPage: (n) => `Стр. ${n}`,
    pagesSelected: (name, n) => `${name} · ${n} стр.`,
    pagesRemaining: (n) => `Останется страниц: ${n}`,
    pagesReset: "Сброс",
    pagesNeedFile: "Сначала выберите PDF",
    pagesAllDeleted: "Все страницы удалены — должна остаться хотя бы одна",
    mergeTitle: "Объединить PDF",
    mergeDesc: "Выберите несколько PDF, упорядочите, скачайте одним файлом.",
    dropText: "Нажмите или перетащите PDF",
    dropTextMulti: (n) => `Выбрано файлов: ${n}`,
    mergeBtn: "Объединить и скачать",
    merging: "Объединение...",
    done: "Загружено",
    error: "Ошибка: ",
    noFile: "Сначала выберите PDF",
    fileListLabel: "Выбранные файлы:",
    remove: "Удалить",
    footer: "100% офлайн · Файлы не отправляются на сервер",
    needTwo: "Нужно минимум 2 PDF",

    splitTitle: "Разделить PDF",
    splitDesc: "Извлеките нужные страницы из PDF и скачайте как новый файл.",
    splitDropText: "Нажмите или перетащите PDF",
    splitDropSelected: (name, total) => `${name} · ${total} стр.`,
    splitRangeLabel: "Диапазон страниц",
    splitRangePlaceholder: "напр: 1-3, 5, 7-9",
    splitRangeHint: "Разделяйте запятыми, диапазон через дефис. Пусто — все страницы.",
    splitBtn: "Разделить и скачать",
    splitting: "Разделение...",
    splitDone: "Загружено",
    splitNeedFile: "Сначала выберите PDF",
    splitInvalidRange: "Неверный диапазон. Пример: 1-3, 5, 7-9",
    splitOutOfRange: (max) => `Номер страницы вне диапазона. В PDF ${max} страниц.`,

    cleanMetaLabel: "Очистить метаданные (автор, программа, даты, GPS)",

    metaToolBtnLabel: "Очистить метаданные",
    metaToolTitle: "Очистить метаданные",
    metaToolDesc: "Удалите скрытую информацию: автора, программу, даты, GPS, заголовок.",
    metaDropText: "Нажмите или перетащите PDF",
    metaDropSelected: (name) => `${name}`,
    metaBtn: "Очистить и скачать",
    metaWorking: "Очистка...",
    metaDone: "Загружено",
    metaNeedFile: "Сначала выберите PDF",
    metaWhatFound: "Найденные метаданные",
    metaWhatClean: "Эта информация будет удалена, и скачается новый PDF.",
    metaNoInfo: "В этом PDF нет заметных метаданных. Всё равно можно очистить.",
    metaAuthor: "Автор",
    metaTitleField: "Заголовок",
    metaSubject: "Тема",
    metaCreator: "Создатель",
    metaProducer: "Производитель",
    metaKeywords: "Ключевые слова",
    metaSuccess: "Метаданные PDF очищены",

    buyCoffee: "Угостить кофе",
    giveStar: "Звезда на GitHub",
    feedback: "Обратная связь",

    mergeSuccess: "PDF успешно объединён",
    splitSuccess: "PDF успешно разделён",
    mergeSuccessClean: "PDF объединён, метаданные очищены",
    splitSuccessClean: "PDF разделён, метаданные очищены",

    themeLight: "Светлая тема",
    themeDark: "Тёмная тема",
    accentBlue: "Синий",
    accentGreen: "Зелёный",
    accentOrange: "Оранжевый",
    accentPink: "Розовый",
    onlyPdf: "Принимаются только PDF файлы",
    oneFileSplit: "Выберите один файл для Разделения",

    statsLine: (files, pages) => `${files} файлов · ${pages} страниц всего`,

    darkReaderWarning: "Обнаружено расширение Dark Reader. Отключите для лучшего вида.",

    previewBtn: "Просмотр",
    previewTitle: (name) => name,
    previewLoading: "Загрузка страниц...",
    previewError: "Не удалось загрузить просмотр",
    previewPage: (n) => `Страница ${n}`,
    previewAllBtn: "Просмотр всех",
    previewAllTitle: (files, pages) => `Все файлы — ${files} файлов, ${pages} страниц`,
    previewFileHeader: (n, name, pages) => `${n}. ${name} · ${pages} стр.`,

    // Изображения в PDF
    imgTitle: "Изображения в PDF",
    imgDesc: "Выберите изображения (JPG, PNG, WEBP), упорядочите и скачайте один PDF.",
    imgDropText: "Нажмите или перетащите изображения",
    imgDropTextMulti: (n) => `Выбрано изображений: ${n}`,
    imgLoading: (n, total) => `Подготовка изображений... ${n}/${total}`,
    imgStats: (n) => `${n} изображений · будет ${n} страниц`,
    imgOnlyImages: "Принимаются только изображения (JPG, PNG, WEBP, GIF, BMP, AVIF)",
    imgHeicUnsupported: (name) => `${name}: формат HEIC не поддерживается. Конвертируйте в JPG.`,
    imgHeicSkipped: (n) => `Пропущено файлов HEIC: ${n} (не поддерживается)`,
    imgLoadError: (name) => `${name} не удалось прочитать, пропущено`,
    imgTooMany: (max) => `Можно добавить не более ${max} изображений`,
    imgMoveUp: "Вверх",
    imgMoveDown: "Вниз",
    imgClear: "Очистить всё",
    imgNeedFile: "Сначала выберите хотя бы одно изображение",
    imgBtn: "Создать PDF и скачать",
    imgWorking: (n, total) => `Создание... ${n}/${total}`,
    imgDone: "Загружено",
    imgSuccess: "PDF создан",
    imgSuccessClean: "PDF создан, метаданные очищены",
    imgPageSize: "Размер страницы",
    imgSizeA4: "A4",
    imgSizeLetter: "Letter",
    imgSizeFit: "По изображению",
    imgOrientation: "Ориентация",
    imgOrientAuto: "Авто",
    imgOrientPortrait: "Книжная",
    imgOrientLandscape: "Альбомная",
    imgMargin: "Поля",
    imgMarginNone: "Нет",
    imgMarginSmall: "Малые",
    imgMarginMedium: "Средние",
    imgQuality: "Качество",
    imgQualityScreen: "Экран",
    imgQualityHigh: "Высокое",
    imgQualityOriginal: "Оригинал",
    imgQualityHint: "Экран: самый маленький файл · Высокое: баланс · Оригинал: лучшее качество (большой файл)",
};
